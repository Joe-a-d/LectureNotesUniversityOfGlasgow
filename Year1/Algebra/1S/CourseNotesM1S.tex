...
$$|\vc{r}_p|^2 = \lambda^2 + \beta^2 + \lambda^2$$
}
...
$$ |\vc{i} \cdot \vc{j}| = |\vc{i}| |\vc{j}| \sin\left(\frac{\pi}{2}\right) = 1$$

and by the rhr (~\ref{cross:rhr}) we also note that $ \vc{i} \cdot \vc{j} = \vc{k}$
}
...
$$\vc{i} \cdot \vc{j} = \vc{k} \quad \vc{j} \cdot \vc{k} = \vc{i} \quad \vc{k} \cdot \vc{i} = \vc{j}$$

and

$$\vc{k} \cdot \vc{j} = -\vc{i} \quad \vc{j} \cdot \vc{i} = \vc{k} \quad \vc{i} \cdot \vc{k} = \vc{j}$$
}
...
\par{In $\real^3$, non-parallel lines need not intersect, they can be \ita{skew} for example varying their "z-index"/"height", shifts the plane they're in and they'll never meet. Hence, by equating two lines, we find}
